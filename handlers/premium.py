...
from utils.database import get_setting, save_payment_request, get_user, is_premium, activate_premium
import os
...
async def show_payment_details(update: Update, context: ContextTypes.DEFAULT_TYPE, plan: str):
    ...
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Orqaga", callback_data="premium_info")]])
    await update.callback_query.edit_message_text(
        "💳 To'lov ma'lumotlari\n\n"
        ...
        "Admin " + payment.get("confirm_time", "1-2 soat") + " ichida tasdiqlaydi.",
        reply_markup=keyboard
    )
...
async def handle_payment_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for admin_id in ADMIN_IDS:
        try:
            await update.get_bot().send_photo(chat_id=admin_id, photo=file_id, caption=admin_text, reply_markup=admin_keyboard)
        except Exception:
            pass
    ...
    await update.message.reply_text(
        ...
        "Admin tez orada tasdiqlaydi. Rahmat!"
    )
